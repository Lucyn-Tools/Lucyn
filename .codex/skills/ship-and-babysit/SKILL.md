...
description: "End-to-end PR shipping workflow for lucyn-ai/lucyn: commit local changes, push to the user's fork, open or reuse a PR against main, then babysit CI and CodeRabbit feedback until the PR is green and clean. Use when the user asks to ship, open a PR, monitor CI, address review comments, or 'babysit' a branch."
---

# Ship and Babysit

Use this skill for `lucyn-ai/lucyn` when the user wants a branch shipped end to end:

- commit the local changes
- push the branch to the user's fork
- open or reuse a PR against `lucyn-ai/lucyn:main`
...
- Follow repo rules from `CLAUDE.md`, including validation and PR checklist requirements.
- Assume `origin` is the user's writable fork and `upstream` points to `lucyn-ai/lucyn`.
- Resolve the fork owner once near the start and reuse it:
  - `FORK_OWNER=$(git remote get-url origin | sed -E 's#.*[:/]([^/]+)/[^/]+(\.git)?$#\1#')`
- If `origin` resolves to `lucyn-ai`, stop and ask the user to add a fork remote. Never push branches to upstream.
...
1. Verify remotes with `git remote -v` and confirm `upstream` points at `lucyn-ai/lucyn`.
...
4. If no PR exists:
   ...
   - create the PR against `lucyn-ai/lucyn:main` with `--head <fork-owner>:<branch>`
...
1. Fetch CI status:
   - `gh pr checks <pr-number> --repo lucyn-ai/lucyn --json name,state,link,description`
...
3. If any check is `FAILURE` or `CANCELLED`:
   - if the `link` is a GitHub Actions run URL, extract the run id and inspect failing logs with `gh run view <id> --log-failed --repo lucyn-ai/lucyn`
...
4. Fetch PR review comments:
   - `gh api repos/lucyn-ai/lucyn/pulls/<pr-number>/comments --paginate`
5. Fetch issue-level PR comments:
   - `gh api repos/lucyn-ai/lucyn/issues/<pr-number>/comments --paginate`
...
Prefer targeted typecheck commands when the touched area is narrow, but do not claim validation passed if a command was not run.
