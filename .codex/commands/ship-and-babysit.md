...
description: Commit local changes, push the branch to the user's fork, open or reuse a PR against lucyn-ai/lucyn:main, then babysit CI and CodeRabbit feedback until the PR is green and clean.
---
...
Use this command when you want an end-to-end ship flow for `lucyn-ai/lucyn`:

1. Commit relevant local changes with a conventional commit.
2. Push the current non-`main` branch to `origin` (the user's fork).
3. Open or reuse a PR against `lucyn-ai/lucyn:main`.
