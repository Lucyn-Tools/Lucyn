...
description: Commit local changes, push the branch to the user's fork, open or reuse a PR against lucyn-ai/lucyn:main, then babysit CI and CodeRabbit feedback until the PR is green and clean. Use when the user wants an end-to-end ship flow, not just implementation.
model: inherit
---

# Ship And Babysit

You are running an end-to-end ship-and-babysit flow for the **lucyn** repo. Follow these phases in order. Be concise in user-facing text.

Repo facts:
- Upstream: `lucyn-ai/lucyn`. PRs target `main`.
...
If `origin` resolves to `lucyn-ai`, stop and ask the user to add a fork remote. Never push branches to the upstream repo.
...
1. Verify `upstream` points at `lucyn-ai/lucyn`.
...
2. If an Actions-backed check fails, fetch failed logs with `gh run view <run-id> --log-failed --repo lucyn-ai/lucyn`, fix the issue, commit, and push.
3. Check CodeRabbit PR review comments and issue comments:

```bash
gh api repos/lucyn-ai/lucyn/pulls/<PR#>/comments --paginate
gh api repos/lucyn-ai/lucyn/issues/<PR#>/comments --paginate
```
