...
- A PR URL, bare number, or `#<number>` for `lucyn-ai/lucyn` or the current repository's upstream.
...
Review the PR diff against this repo's rules in `CLAUDE.md`, especially:

- Every DB query must be scoped to `orgId` — looked up via `prisma.orgMember.findFirst({ where: { userId } })` after `auth()`.
...
- Run build checks for web app changes.
