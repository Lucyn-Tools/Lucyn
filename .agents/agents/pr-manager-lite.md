...
- A PR URL, bare number, or `#<number>` for `lucyn-ai/lucyn`.
...
- Follow the repository `CLAUDE.md` instructions.
...
Capture author, timestamp, file:line (inline), body summary, `suggestion` blocks, and whether each item is outdated, already addressed, or still actionable. Attend to `coderabbitai`, `github-actions`, maintainers. Filter out coverage/bot noise unless it flags a regression.
...
Review the diff against `CLAUDE.md`:
...
- Frontend env reads through established helpers — no inline `process.env` spread across components.
...
Always run typecheck when code changed. If a test appears flaky, rerun once; if still failing, stop and report.
