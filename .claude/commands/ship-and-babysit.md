---
description: Commit, push to origin (fork), open PR to lucyn-ai/lucyn:main, then poll every ~5min for CodeRabbit comments and CI failures, resolve them, and exit when clean.
allowed-tools: Bash, Read, Edit, Write, Agent, Skill
---

You are running an end-to-end ship-and-babysit flow for the **lucyn** repo. Follow these phases in order. Be concise in user-facing text — one short sentence per phase transition is enough.

Repo facts (from `CLAUDE.md`):
- Upstream: `lucyn-ai/lucyn` (not a fork). PRs target **`main`**.
- Push branches to **`origin`** (the user's own fork of `lucyn-ai/lucyn`). Treat `upstream` as fetch-only.
- PRs are opened with `--head <fork-owner>:<branch>` against `lucyn-ai/lucyn:main`.
...
```
The flow is **fork-only**: `origin` must be the user's fork. If `origin` resolves to `lucyn-ai` (the upstream org), stop and ask the user to add a fork remote — never push branches to the upstream repo.
...
1. Verify upstream remote with `git remote -v`. It should point at `lucyn-ai/lucyn`. If missing, ask the user before adding it.
2. Check whether a PR already exists for this branch:
   `gh pr list --repo lucyn-ai/lucyn --head <fork-owner>:<branch> --state open --json number,url`
...
4. Create the PR:
   ```bash
   gh pr create --repo lucyn-ai/lucyn --base main --head <fork-owner>:<branch> \
     --title "..." --body "$(cat <<'EOF'
...
1. **Fetch CI status**:
   `gh pr checks <PR#> --repo lucyn-ai/lucyn --json name,state,link,description`
   - `gh pr checks --json` returns a `link` field (an Actions URL like `…/actions/runs/<id>/job/<jobId>`), not a run id directly. Extract the run id with a regex that's robust to trailing slashes (`sed -nE 's#.*/actions/runs/([0-9]+)/.*#\1#p'`) — positional `awk -F/` is brittle when the URL has a trailing slash. Or skip URL parsing entirely and call `gh run list --repo lucyn-ai/lucyn --branch <branch> --json databaseId --limit 1 --jq '.[0].databaseId'`.
   - If any check is `FAILURE` or `CANCELLED`, branch by check type: when `link` matches `/actions/runs/<id>/` (Actions-backed), extract `<id>` and fetch logs with `gh run view <id> --log-failed --repo lucyn-ai/lucyn`; when it doesn't (e.g. the `CodeRabbit` virtual check or any other status posted directly via the Checks API without an Actions run), skip `gh run view` and work from the `name`/`state`/`description` fields plus any review comments. Then fix the underlying issue: edit code, commit (conventional prefix), push to `origin`. Do NOT skip hooks or disable failing tests to make CI green.
...
2. **Fetch CodeRabbit review comments**:
   `gh api repos/lucyn-ai/lucyn/pulls/<PR#>/comments --paginate`
   Filter for comments authored by `coderabbitai` / `coderabbitai[bot]`. Also check issue-level comments: `gh api repos/lucyn-ai/lucyn/issues/<PR#>/comments --paginate`.
   - For each unresolved CodeRabbit suggestion: read the file/line referenced and apply the fix if it is correct and in scope. If a suggestion is wrong or out of scope, reply *inside the existing thread* (so the reply attaches to the same conversation, not a brand-new review) before resolving:
     ```bash
     gh api repos/lucyn-ai/lucyn/pulls/comments/<comment_id>/replies \
       -X POST \
       -f body='**Dismissed:** <reason>'
     ```
     (`<comment_id>` is the top-level review-comment id from `gh api repos/lucyn-ai/lucyn/pulls/<PR#>/comments`. `POST /pulls/<PR#>/reviews` would create a *new* review thread, not a reply.)
...
- Never push to `upstream` (`lucyn-ai/lucyn`) — only to `origin` (the user's fork). Treat upstream as fetch-only.
