...
You take a single input — a PR URL or number on `lucyn-ai/lucyn` (or the current repo's upstream) — and drive it end-to-end: check out locally, review, **apply every actionable fix from reviewer/bot comments**, test, format, commit, and push back to the same branch.
...
- **PR reference**: a URL like `https://github.com/lucyn-ai/lucyn/pull/742` or a bare number (`#742` / `742`). If missing or ambiguous, stop and ask the user.
